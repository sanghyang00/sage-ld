...
from dataset import EELDDataset
...
def main():
    ...
    parser.add_argument('--data_dir', type=str, default='simulated')
    parser.add_argument('--noise_file_path', type=str, default='noise_file_list.txt')
    parser.add_argument('--rir_file_path', type=str, default='rir_file_list.txt')
    parser.add_argument('--add_rir', type=str2bool, default='True')
    parser.add_argument('--add_noise', type=str2bool, default='True')
    parser.add_argument('--vad', type=str2bool, default='False')
    parser.add_argument('--pretrained', type=str2bool, default='True')
    parser.add_argument('--ckpt_path', type=str, default='pretrained_ckpts/mms_feature_extractor.pt')
    ...
    SAVE_DIR = f'pretrained_ckpts'
    REMOVE_MUTE =  not args.vad
    os.makedirs(SAVE_DIR, exist_ok=True)

    data_dir = args.data_dir
    noise_files = args.noise_file_path
    rir_files = args.rir_file_path
    ...
    if args.pretrained:
        print('Loading feature extractor parameters...')
        encoder.feature_extractor.load_state_dict(torch.load(args.ckpt_path, map_location='cpu'))
    ...
    train_dataset = EELDDataset(data_dir, noise_files, rir_files, mode='train', add_rir=args.add_rir, add_noise=args.add_noise, snr_db=[5,10,15,20])
    val_dataset = EELDDataset(data_dir, noise_files, rir_files, mode='dev', add_rir=args.add_rir, add_noise=args.add_noise, snr_db=[5,10,15,20])
