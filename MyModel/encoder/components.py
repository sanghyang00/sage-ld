...
class CrossAttention(Module):
    ...
    Args:
        embed_dim (int): Total dimension of the model (query/key/value).
    ...
    def forward(
        self,
        query: Tensor,                 # [B, Lq, D]
        key_value: Tensor,            # [B, Lkv, D]
        attention_mask: Optional[Tensor] = None,  # [B, 1, Lq, Lkv]
    ) -> Tuple[Tensor, Optional[Tensor]]:
        """
        Args:
            query (Tensor): shape [batch_size, query_len, embed_dim]
            key_value (Tensor): shape [batch_size, key_value_len, embed_dim]
            attention_mask (Tensor or None): shape [batch_size, 1, query_len, key_value_len]
        Returns:
            (Tensor, None): shape [batch_size, query_len, embed_dim]
        """
        B, Lq, _ = query.shape
        B_kv, Lkv, _ = key_value.shape
        if B != B_kv:
            raise ValueError("Batch size of query and key/value must match")

        # Project
        q = self.q_proj(query).view(B, Lq, self.num_heads, self.head_dim).transpose(1, 2)  # [B, H, Lq, Hd]
        k = self.k_proj(key_value).view(B, Lkv, self.num_heads, self.head_dim).transpose(1, 2)  # [B, H, Lkv, Hd]
        v = self.v_proj(key_value).view(B, Lkv, self.num_heads, self.head_dim).transpose(1, 2)  # [B, H, Lkv, Hd]
...
class DecoderLayer(Module):
    ...
    def __init__(
        ...
        self.self_attn_layer_norm = nn.LayerNorm(self_attention.embed_dim)
        self.cross_attn_layer_norm = nn.LayerNorm(cross_attention.embed_dim)
        self.final_layer_norm = nn.LayerNorm(cross_attention.embed_dim)

    def forward(
        self,
        x: Tensor,                          # decoder input: [B, Lq, D]
        encoder_output: Tensor,            # encoder output: [B, Lkv, D]
        self_attn_mask: Optional[Tensor] = None,
        cross_attn_mask: Optional[Tensor] = None,
        self_position_bias: Optional[Tensor] = None,
        cross_position_bias: Optional[Tensor] = None,
        ...
        x, self_position_bias = self.self_attention(
            x,
        ...
        x, cross_position_bias = self.cross_attention(
            x,
            encoder_output,
